  ...
  const bundleTitle = () => {
    ...
    return (
      <div className={styles.singleItem}>
        ...
        >
          <div className={styles.priceContainer}>
            <div className={`${textStyles.m_16} ${styles.whiterText}`}>
              {getTotalAmount()}$
            </div>
          </div>
            ...
            <div className={`${textStyles.m_16} ${styles.whiterText}`}>
              {tokenSymbol.length > 3
                ? `${tokenSymbol
                    .slice(0, 2)
                    ?.toString()
                    ?.replace(/,/g, "+")} & ${tokenSymbol?.length - 3} others`
                : `${tokenSymbol.slice(0, 2)?.toString()?.replace(/,/g, "+")}`}
            </div>
          </div>
        </div>
        <div className={styles.addressContainer}>
          <div className={styles.bundleInfo}>
            <div className={`${textStyles.m_16} ${styles.whiterText}`}>
              {moment(item?.gnosis?.submissionDate).startOf("hour").fromNow()}
            </div>
            ...
            >
              ...
              >
                ...
                >
                  {item?.gnosis.confirmations?.length}/{delegates.length}
                </div>
              </div>
          ...
          <div style={{ flexDirection: "row", display: "flex", width: "60%" }}>
            ...
            {!item?.gnosis?.isExecuted && (
              <div
                ...
                style={{ background: getButtonProperty()?.background }}
                className={styles.btnContainer}
              >
                <div
                  style={{ color: getButtonProperty()?.color }}

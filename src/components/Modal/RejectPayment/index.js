...
import {getPayoutRequest, set_payout_filter, syncTxDataWithGnosis } from '../../../store/actions/dao-action'
    ...
    return(
        <div className={styles.backdrop}>
            <div className={styles.modal}>
                <img onClick={onClose} alt='cross' src={cross} className={styles.cross} />
                <div style={{color:'black', textAlign:'start', marginBottom:'2.5rem'}} className={textStyles.ub_36}>
                ...
                <div className={styles.contibutionDiv}>
                    <div className={styles.contibutionTitle}>
                        <span className={textStyles.m_19}>
                            {currentPayment?.metaInfo?.contributions.length>1?`Bundle Payment ${currentPayment?.metaInfo?.contributions?.length}`:currentPayment?.metaInfo?.contributions[0].title}
                        </span>
                        <span className={textStyles.m_19}>
                            {getContriInfo()?.total}$
                        </span>
                    </div>
                    <span style={{textAlign:'start'}} className={textStyles.m_19}>
                    {getContriInfo().totalToken}
                    </span>
                </div>
                <div className={styles.btnCnt}>
                    <div onClick={()=>dispatch(setRejectModal(false))} style={{background:'#E0E0E0'}} className={styles.btnDiv}>
                        <div className={textStyles.ub_16}>
                            Cancel
                        </div>
                    </div>
                    <div onClick={async()=>await rejectTransaction(currentPayment?.gnosis?.safeTxHash)} style={{background:'#FF0000'}} className={styles.btnDiv}>

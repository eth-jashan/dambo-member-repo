...
const DeployGnosisButton = () => {
    ...
    const isSafeOwnerConnected = owners.includes(
        ...
        const partialTx = [
            {
                ...
                data: "0x",
                value: ethers.utils
                    .parseEther(value ? value.toString() : "0.001")
                    .toString(),
            },
            {
    ...
    const executeSafeTransaction = useCallback(
        async (transaction) => {
            ...
            const safeTransactionData = {
                ...
                safeTxHash:
                    "0xe1190d731bf83d0ac22dd1dd47246b6df66e954eb85f18a324377476783025b8",
    ...
    const renderSigners = () =>
        transaction.map((item) => (
            <>
                {!isTransactionSignedByAddress(item) ? (
                    <Button
                        style={{ marginBottom: 20 }}
                        onClick={async () => await confirmTransaction(item)}
                        type="primary"
                    >
                        Sign Tx
                    </Button>
                ) : isTransactionExecutable(item) ? (
                    <Button
                        onClick={() => executeSafeTransaction(item)}
                        type="primary"
                    >
                        Execute Tx {item.value / 1000000000000000000}Ethers{" "}
                    </Button>
                ) : (
                    `waitingggg for more signers for ${
    ...
    return (
        <div style={{ flexDirection: "column", display: "flex" }}>
            {safeAddress ? (
                ...
                <Button
                    loading={deploying}
                    onClick={async () => await deploySafe(OWNERS, THRESHOLD)}
                    type="primary"
                >
                    Deploy Safe
                </Button>
            ...
            {safeBalance ? (
                ...
                >
                    {safeBalance?.toString() / 1000000000000000000} Ethers
                </Typography.Text>
            ) : null}
            {transaction.length > 0 && (
                <Typography.Text>{transaction.length}</Typography.Text>
            )}
            {renderSigners()}
        </div>
    )
